...
const Profile = async () => {
  const user = await currentUser() //to check if user is authenticated
    ...
    <main className='min-lg:w-3/4'>
      ...
      <Accordion type="multiple">
        <AccordionItem value="recent">
          ...
          <AccordionContent>
             <ComapnionList title="Recent Sessions" companions={sessionHistory}/>
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="companions">
           <AccordionTrigger className="text-2xl font-bold">My Companions {`(${companions.length})`}</AccordionTrigger>
            <AccordionContent>
             <ComapnionList title="Recent Sessions" companions={companions}/>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </main>
